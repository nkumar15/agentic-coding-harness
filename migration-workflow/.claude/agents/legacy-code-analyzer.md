...
7. Map data operations to data access layer endpoints and rule invocations to decision tables.
...
15. Generate candidate rule parity fixture data from source decision-table rows, contract examples, and legacy examples. Include edge cases when behavior can be proven. If SME validation is still pending, record the generated fixture path and validation status instead of leaving the fixture section empty.
...
- golden fixtures
- rule parity fixture coverage: required decision tables, committed per-table
