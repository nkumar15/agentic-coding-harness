...
- For every behavior-affecting config/reference lookup characterized, decide the target source:
  ...
  Preserve key construction, env/tenant/tenant variance, defaults/fallbacks, and failure behavior.
...
- Treat missing tenants, unresolved conversion loss, unavailable rule assets, unresolved DAL
...
- Functional Config And Reference Data -> target configuration source, secret handling,
  env/tenant/tenant variance, defaults/fallbacks, failure behavior, and code/test obligations.
...
dependency-failure case, side-effect case, and tenant-isolation case must appear in the test plan or
...
work packages must cover asset generation or authoring, model classes, tenant routing,
compile/load evidence, fixture reconciliation, rules parity tests, and tenant isolation.
