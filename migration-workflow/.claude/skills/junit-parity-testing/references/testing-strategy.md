...
| Rules parity tests | Feed curated legacy decision-table fixture rows into the migrated rule implementation and compare outputs for the active domain, including tenant isolation. | `legacy-parity-verifier` reports; migrator or analyze fixes by cause | `rules_parity_verified` |
...
| tenant-specific rule behavior | Rules parity by tenant plus cross-tenant isolation guard |
...
- [ ] Rule invocation inputs/outputs and tenant isolation when rules are touched.
...
- Fixture files under the project-declared rules fixture path are the parity contract. A common
  pattern is `tests/parity-data/rules/<DecisionTable>/<TenantOrtenant>.json`.
...
- Candidate fixture data and the per-tenant source rule-count matrix should be generated during
...
- Every `domain-required` and `shared-required` table must have fixture coverage for every
  applicable tenant, with one file per decision-table/tenant pair, or an approved documented gap.
- Duplicate normalized inputs with conflicting outputs, unsupported helper/operator translations,
  broad-overwrite risks, or missing tenant scope block parity until classified.
- Existing rule assets under repository `rules/` must have migration-process verification before
  ...
  model compatibility, source/fixture/test reconciliation, and tenant isolation.
- Cross-tenant isolation must prove one tenant's rules do not fire for another legal entity.
...
- Every tenant-specific rule path has tenant-isolation coverage.
