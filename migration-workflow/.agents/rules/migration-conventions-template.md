...
| Source decision table paths | tenant/tenant: `<FILL_IN: e.g. legacy/myapp-legacy/framework/rules/*TenantRules/DecisionTables/*.rules>`; shared/common: `<FILL_IN: e.g. legacy/myapp-legacy/framework/rules/CommonRules/DecisionTables/*.rules — or "none">` |
| Rule fixture generator | `<FILL_IN: e.g. tools/decision-table-parser/parse_decision_tables.py — or "none">` |
| Rule fixture path pattern | `<FILL_IN: e.g. tests/parity-data/rules/<DecisionTable>/<tenant>.json>` |
...
Decision tables: `<FILL_IN: glob for decision table files per tenant/tenant, e.g. Tenant{A,B,C}Rules/DecisionTables/*.rules>`
...
Shared/common projects are not tenants/tenants; classify them separately when they are required or
transitively required by a migration unit.
`<FILL_IN: any tenant-specific extra tables, e.g. "tenant C has extra region-specific decision tables" — or delete this line>`.
...
- **Ignore the repository rules directory during analysis inventory.** Do not use migrated rule assets
  to derive source rule counts, fixture rows, tenant coverage, or parity expectations.
...
- Generate rule parity fixture candidates from the legacy decision-table source with
  ...
  The parser emits one JSON file per decision-table/tenant pair under
  `<FILL_IN: fixture output path, e.g. tests/parity-data/rules/<DecisionTable>/<tenant>.json>`,
  ...
  as a tenant/tenant column.
  `<FILL_IN: note any deprecated parser and why — or delete this line>`.
- Keep shared/common rule fixtures distinct from tenant/tenant fixtures. Do not represent common
  rows as a tenant/tenant column, and do not let same-named common and tenant/tenant decision tables
...
across every applicable tenant/tenant, including shared/common and transitive tables.

Decision-table classification (use in characterization reports):

| Classification | Meaning | Required handling |
| --- | --- | --- |
| `domain-required` | Directly invoked by the unit's legacy orchestration path. | Must have migrated asset, fixture/parity coverage for every applicable tenant. |
...
regression gate (see `.agents/process/gates.yaml`).
...
- **Rules parity** (gate `rules_parity_verified`, tag `rules-parity`) — OFFLINE, no app/deploy,
  ...
  Golden rows in
  `<FILL_IN: rules fixture path pattern, e.g. tests/parity-data/rules/<DecisionTable>/<tenant>.json>`
...
4. **No rule fires for the wrong `<FILL_IN: tenant/tenant>` — zero cross-tenant leakage.** Enforce
