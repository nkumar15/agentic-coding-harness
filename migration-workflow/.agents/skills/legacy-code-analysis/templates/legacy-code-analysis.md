...
| --- | --- | --- | --- |
| Business SME / product owner | Executive Review; Functional Behavior Review; Error Codes And Messages; Open Questions / Decisions Required | Business outcomes, tenant differences, user-visible errors, in/out of scope behavior | Whether the characterized behavior matches intended business behavior |
...
- tenant-specific behavior summary:
...
- [ ] Functional config/reference-data lookups are traced from lookup key construction through
      value source, consumers, env/tenant/service variance, fallback/default behavior, and target
      migration action.
...
- [ ] Rule corpus inventory covers every applicable tenant rule project, plus shared/common rule
...
- [ ] Candidate rule parity fixtures are generated from the authoritative decision-table source for
      every `domain-required`, `shared-required`, and `unknown` table across applicable tenants and
      shared/common rule projects.
- [ ] A per-tenant decision-table rule-count matrix is included for required/shared/unknown tenant
...
| Operation | Business decision / outcome | Inputs affecting decision | Outputs affected | tenant differences | Rule/dependency source | Evidence confidence |
| --- | --- | --- | --- | --- | --- | --- |
| `<operation>` | `<decision/outcome>` | `<fields>` | `<response fields/side effects>` | `<tenant-specific behavior>` | `<rule/flow/adapter>` | `<proven/inferred/not-found/open-question>` |
...
### tenant Behavior Matrix

| tenant | Behavior variant | Required rule tables / dependencies | Known gaps | Evidence confidence |
| --- | --- | --- | --- | --- |
| `<tenant>` | `<same/different behavior>` | `<tables/dependencies>` | `<gap or none>` | `<proven/inferred/not-found/open-question>` |
...
| Rule area | Business meaning | In-scope tenants | Required tables | Fixture status | Conversion fidelity status | Evidence confidence |
| --- | --- | --- | --- | --- | --- | --- |
| `<pricing/config/rates/etc.>` | `<business purpose>` | `<tenants>` | `<tables>` | `<covered/partial/blocker>` | `<pass/partial/blocker/not available>` | `<proven/inferred/not-found/open-question>` |
...
feature switches, tenant mappings, downstream URL selection, rule inputs, peer/interface/event
...
| Lookup / key | Used by | Key construction evidence | Value source evidence | Value consumers / behavior impact | Env / tenant / tenant / service variance | Default / fallback / missing behavior | Target migration action | Evidence confidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
exists. But before excluding any table, inventory every tenant rule project and shared/common rule
project checked for this domain and classify every decision table. Shared/common rule projects are
not tenants; keep them out of the per-tenant matrix and report them separately.

### tenant Rule Project Inventory

| tenant | Legacy decision-table project | Source decision-table count checked | Migrated rule impl count checked | Fixture count checked | Stale/non-authoritative source ignored? | Notes | Evidence confidence |
| --- | --- | ---: | ---: | ---: | ---: | --- | --- |
| `<tenant>` | `<path>` | `<count>` | `<count>` | `<count>` | `<yes/no/exception id>` | `<missing/extra assets>` | `<proven/inferred/not-found/open-question>` |
...
reaches a non-tenant rule project. Do not represent these rows as tenant columns.
...
Classify every decision table found in the checked tenant rule projects and shared/common rule
projects. `unknown` blocks design. `domain-required` and `shared-required` must be carried into
design and migration for every applicable tenant or shared/common project.

| Decision table | Source scope | tenants/common projects present | Classification | Why this classification is correct | Direct source evidence | Required for this domain? | Exclusion / follow-up | Evidence confidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant/shared-common/both>` | `<tenants and/or common projects>` | `<domain-required/shared-required/used-by-other-domain/not-used-by-this-domain/unknown/excluded-with-approved-reason>` | `<reason>` | `<flow/rule/service paths>` | `<yes/no/blocker>` | `<approved exclusion or open question>` | `<proven/inferred/not-found/open-question>` |
...
same-named table exists in both tenant and shared/common projects, keep the evidence and fixture
paths separate.

| Operation / legacy service | Business-rule service or lookup | Active rule project evidence | Decision table | tenants/common projects required | Direct or shared/transitive | Inputs consumed | Outputs used by domain | Evidence confidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<operation/service>` | `<call*Rule/config lookup>` | `<namespace/project-name/routing evidence>` | `<table>` | `<tenants/common projects>` | `<direct/shared-transitive>` | `<inputs>` | `<outputs>` | `<proven/inferred/not-found/open-question>` |

### Required Rule Asset Coverage

For every `domain-required` and `shared-required` table, prove coverage for every applicable tenant.
...
tenant decision-table matrix:

| Decision table | `<tenant 1>` | `<tenant 2>` | `<tenant ...>` | Total |
| --- | ---: | ---: | ---: | ---: |
...
| tenant | Required DT count | Required rule impl count | Required source decision-table count | Required fixture count | Missing required tables/assets | Coverage verdict |
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `<tenant>` | `<count>` | `<count>` | `<count>` | `<fixture file count and row count>` | `<table/asset gaps>` | `<covered/partial/blocker>` |
...
| Rule table | tenant | Candidate `rules/` asset | Compile/load evidence | Module/package routing | Model compatibility | Source reconciliation | Fixture/test reconciliation | tenant isolation | Verdict |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant>` | `<path>` | `<pass/fail/not-run>` | `<pass/fail/gap>` | `<pass/fail/gap>` | `<source rows/actions/conditions reconciled or blocker>` | `<fixtures/tests reconciled or blocker>` | `<pass/fail/not-run>` | `<verified/verified-with-approved-gaps/blocker>` |
...
| Decision table | Required tenants | Candidate fixture path pattern | Source parser / command | Generation status | SME validation status |
| --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenants>` | `tests/parity-data/rules/<DecisionTable>/<tenant>.json` | `<command>` | `<generated / failed / not-run>` | `<validated / pending / blocker>` |

Rule parity fixture coverage for decision-table-backed behavior:

| Decision table | tenant | Committed fixture | Source rule count | Fixture rule count | Validation status |
| --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant>` | `tests/parity-data/rules/<DecisionTable>/<tenant>.json` | `<count>` | `<count>` | `<validated / corrected / blocker>` |
...
| Decision table | tenant | Source decision table | Rule asset | Fixture/test evidence | Row/rule/test reconciliation | Helper/operator coverage | Conflict/overwrite audit | Activation semantics | Implementation-shape evidence | Verdict |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant>` | `<path>` | `<path or missing>` | `<fixture/test path>` | `<source=N fixture=N impl=N tests=N or exclusions>` | `<all translated / blockers>` | `<none / duplicate inputs / broad overwrite>` | `<independent / sequential / first-match / unknown>` | `<compile/load, module/package, tenant-isolation verdict>` | `<pass / pass-with-exclusions / blocker>` |

Conversion blockers:

- `<table/tenant/rule ids, source path, lost condition/action/helper/order behavior, impact, owner>`
...
| Functional config and reference data | Target source for behavior-affecting config, secret handling, env/tenant/tenant variance, defaults/fallbacks, and failure behavior | Spring configuration properties, secret references, DAL/reference-data calls, or approved static values plus tests |
...
| Rules and conversion fidelity | Audited rule implementations/model classes, tenant isolation, and conversion-remediation work packages | Rule wiring and rules parity tests only for audited assets |
...
- Rule integration with tenant isolation.
