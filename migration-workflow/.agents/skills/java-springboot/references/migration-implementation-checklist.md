...
- [ ] Confirm rule assets to be wired have conversion-fidelity evidence, implementation-shape
      verification, and tenant coverage.
...
- [ ] Wire only audited rule assets with implementation-shape verification and preserve tenant
      isolation.
- [ ] Complete required rule-asset remediation work before functional code that depends on those
      rules; do not replace missing rules with hardcoded constants, partial tenant shortcuts, or DAL
...
- [ ] No silent skipped tests, missing fixtures, or partial tenant/rule coverage.
- [ ] No hardcoded hosts, secrets, tenant ids outside approved constants/config, or direct database
      calls.
