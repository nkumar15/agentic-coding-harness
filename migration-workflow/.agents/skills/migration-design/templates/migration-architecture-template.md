...
| tenant Behavior Matrix | `<tenant differences>` | Rules Design / Dependency Behavior / Test Plan | `<decision/test/gap>` | `<status>` |
...
| Rule Corpus / Required Rule Asset Coverage | `<tables/tenants/fixtures/assets>` | Rules Design / Rule Asset Gap And Remediation Plan | `<decision/WP/test/gap>` | `<status>` |
...
- [ ] Functional config/reference-data lookups have target source decisions, ownership,
      secret-safety handling, env/tenant/tenant variance, defaults/fallbacks, and test obligations.
...
- [ ] Rule assets, model classes, tenant routing, fixture coverage, and decision-table-to-target
...
- [ ] Every characterized happy path, edge case, negative case, dependency-failure case,
      side-effect case, and tenant-isolation case maps to a test obligation or design gap.
...
| `<migrated rules directory from migration conventions>` | `<migrated tenant rule assets>` | `<rule/model assets>` | `<consumed by rule module>` | `<review/staging folders are not target structure unless approved here>` |
...
Rule-asset location and tenant routing:
...
| Legacy lookup / key | Behavior impact | Legacy source evidence | Target source / owner | Target property / endpoint / fixture | Secret handling | Env / tenant / tenant variance | Default / fallback / missing behavior | Tests / gates | Decision / Gap |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| Rule Table | tenant | Source Decision Table | Rule Asset | Model Class | Result Class | Inputs | Outputs | Gap / Decision |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant>` | `<legacy decision-table path>` | `<path>` | `<class>` | `<class>` | `<inputs>` | `<outputs>` | `<gap/decision>` |
...
compatibility, fixture/test reconciliation, and tenant isolation. Stale/non-authoritative
...
| Rule Table | tenant | Characterization Verdict | Source Decision Table / Implementation / Test Reconciliation | Implementation-Shape Verification | Helper / Operator Translation | Conflict / Overwrite Risk | Design Decision | Required Work Package |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<table>` | `<tenant>` | `<pass/blocker>` | `<counts/evidence>` | `<compile/load, module/package, tenant isolation>` | `<status>` | `<status>` | `<wire/remediate/gap>` | `<WP-id>` |
...
| Gap | Affected Tables / tenants | Required Remediation | Target Files / Assets | Acceptance Evidence | Work Package | Blocks |
| --- | --- | --- | --- | --- | --- | --- |
| `<missing rule impl/model/routing/tests>` | `<tables/tenants>` | `<generate/author/reconcile/wire>` | `<paths/classes>` | `<compile/load, fixture reconciliation, rules parity, tenant isolation>` | `<WP-id>` | `<WP-id or function>` |
...
- [ ] Rule assets wired by the work package have passing decision-table-to-target conversion-fidelity
      evidence, implementation-shape verification, and tenant isolation.
...
paths, edge cases, negative cases, dependency failures, side effects, and tenant-isolation cases.
...
- [ ] Rule assets, model classes, tenant gaps, and decision-table-to-target conversion-fidelity
