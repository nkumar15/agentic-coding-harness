...
- Apply `.agents/rules/migration-conventions.md` for project source paths, contract rules,
  rule-source priority, DAL boundaries, parity expectations, and hard project constraints.
- Apply `.agents/rules/llm-behavior.md` for cautious, surgical analysis.
- Apply `.agents/rules/command-execution.md` for shell command discipline.
...
9. For config/reference lookups that affect functional behavior, trace key construction, source
   evidence, consumers, env/tenant/tenant/service variance, defaults/fallbacks, and target migration
...
13. Infer whether existing target implementation exists for the domain from project conventions,
    ...
    assumptions and proven legacy behavior, including hardcoded tenant data, missing branch paths,
...
14. For rule behavior, use only legacy decision-table artifacts under the legacy decision table
    ...
    derive rule counts, fixtures, tenant coverage, or behavior unless a human explicitly approves a
    ...
    Keep shared/common rule projects separate from tenant/tenant rule projects; never report them
    as a tenant column. When a rule wrapper targets a same-named decision table in multiple rule
...
15. Generate candidate rule parity fixture data from source decision-table rows with
    ...
    `shared-required`, and `unknown` table across applicable tenants and shared/common rule
    projects. Keep parser output under the project-declared per-table/per-tenant fixture path
    pattern, normally `tests/parity-data/rules/<DecisionTable>/<tenant>.json`, with tenant metadata
    ...
    `tests/parity-data/rules/common/<DecisionTable>.json`, and do not report common as a tenant.
    Include the generated per-tenant rule-count matrix and any separate shared/common rule-count
...
19. Verify rule fixture coverage before analyze approval: every in-scope decision table has a
    committed fixture file for every in-scope tenant, and fixture rule counts reconcile with legacy
...
`.agents/skills/legacy-code-analysis/templates/legacy-code-analysis.md`. The report must keep the
...
- golden fixtures
- rule parity fixture coverage: required decision tables, tenants, committed per-table/per-tenant
...
per-table/per-tenant rules fixtures are missing or SME validation/correction is pending for in-scope
decision tables or tenants.
