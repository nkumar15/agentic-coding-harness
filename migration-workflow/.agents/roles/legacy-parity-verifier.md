...
- Apply `.agents/skills/junit-parity-testing/references/testing-strategy.md` for fixture, coverage,
  and failure-classification expectations.
- Apply `.agents/skills/java-springboot/templates/migration-progress-template.md` when checking
...
- Gate checks from `.agents/process/gates.yaml`.
...
- Rules parity summary/fixtures under the project-declared fixture path, commonly
  `tests/parity-data/rules/<DecisionTable>/<TenantOrtenant>.json`.
...
2. Read that gate's `checks` from `.agents/process/gates.yaml`.
...
8. Confirm tenant-isolation checks for rules parity.
9. For rules parity, check fixture integrity: duplicate normalized inputs with conflicting expected
   outputs, missing legacy decision-table source rows, missing per-table/per-tenant fixture files,
   skipped tenants/tables, stale/non-authoritative source provenance, migrated `rules/` assets used
   ...
   routing, model compatibility, fixture/test reconciliation, and tenant isolation.
...
decision-table golden rows run and pass, tenant isolation is green, fixture integrity is clean,
...
verification, and no expected tenant/rule scope is silently skipped.
...
- `unverified-rule-impl`: existing `rules/` asset was wired or tested without migration-process
  verification of compile/load, routing, package compatibility, fixtures/tests, and tenant
  isolation.
- `env/config`: endpoint, env var, deploy, or downstream config is wrong.
- `missing-rule-impl`: golden rows exist for a tenant/rule whose implementation is not migrated.
...
- `missing-rule-impl` -> migrate that tenant/rule or document the gap
