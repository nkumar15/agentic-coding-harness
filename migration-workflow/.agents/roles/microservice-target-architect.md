...
- Apply `.agents/rules/migration-conventions.md` for project stack, contract, DAL, rule-source,
  parity, and hard constraints.
- Apply `.agents/rules/llm-behavior.md` and `.agents/rules/command-execution.md`.
...
- Characterized functional config/reference-data findings, including lookup keys, source evidence,
  consumers, env/tenant/tenant/service variance, defaults/fallbacks, and secret-safety notes.
...
9. Translate functional config/reference-data findings into explicit target decisions: Spring
   ...
   construction, env/tenant/tenant variance, defaults/fallbacks, and failure behavior. Do not let
...
16. Define the parity plan by tracing every characterization scenario and edge/failure case to a
    ...
    side-effect cases and cross-tenant isolation.
17. For each rule asset, state whether it is audited and safe to wire, needs conversion
    ...
    model compatibility, fixture/test reconciliation, and tenant isolation before any story wires
...
18. When characterization has source decision tables and fixtures but no acceptable rule asset,
    ...
    tenant routing, compile/load evidence, fixture reconciliation, rules parity tests, and
    tenant isolation before any functional story depends on those rules.
19. Use the canonical architecture template from
    `.agents/skills/migration-design/templates/migration-architecture-template.md`; do not copy an
    older domain artifact as the source of structure.
20. Use `.agents/skills/junit-parity-testing/references/testing-strategy.md` to map every
...
`.agents/skills/migration-design/templates/migration-architecture-template.md`.
...
- rule assets, model classes, and tenant gaps
- decision-table-to-target conversion fidelity status for each rule table/tenant, including any remediation work
...
designing over the gap. Missing tenants, unavailable rule implementations, existing `rules/` assets without
