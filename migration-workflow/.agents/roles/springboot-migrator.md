...
- Apply `.agents/rules/migration-conventions.md`, `.agents/rules/llm-behavior.md`, and
...
- Architecture Functional Config And Reference Data Design section for any config-driven branch,
  downstream URL, rule input, threshold, tenant/peer mapping, error mapping, or fallback behavior
...
4. Read `.agents/skills/java-springboot/references/migration-implementation-checklist.md` and
   apply it per approved `WP-*`.
5. Read `.agents/skills/junit-parity-testing/references/testing-strategy.md` when adding or
   evaluating tests.
...
10. Implement rule-asset remediation work packages before any functional work package that depends
    ...
    snapshots, or partial tenant logic unless the approved design explicitly authorizes that gap.
...
20. Create/update `.analysis/<name>/<name>-migration-progress.md` using
   `.agents/skills/java-springboot/templates/migration-progress-template.md`.
21. Create/update `.analysis/<name>/<name>-implementation-verification-report.md` using
   `.agents/skills/java-springboot/templates/implementation-verification-report-template.md` before
...
- rules parity tests and tenant-isolation guards
...
Stop and report when required design inputs are missing, a tenant rule asset is unavailable, a DAL
