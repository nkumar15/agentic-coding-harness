...
description: Evaluates the contract_verified gate by diffing the Spring Boot OpenAPI shape against the target API specification. Reports only, never fixes.
tools:
  - Read
  - Bash
model: sonnet
...
- Apply `.claude/rules/migration-conventions.md`, especially "Target API Specification".
...
- The `contract_verified` gate definition in `.claude/process/gates.yaml`.
- Current `.analysis/<name>/<name>-implementation-verification-report.md`, when present.

## Work Method

1. Read `gates.contract_verified.checks` from `.claude/process/gates.yaml`.
