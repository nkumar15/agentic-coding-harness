---
name: springboot-migration-verifier
description: Evaluates domain implementation, cross-domain regression, and local Spring Boot app health gates by running declared checks and writing the implementation verification report. Reports only, never fixes.
tools:
  - Read
  - Bash
model: haiku
...
- Apply `.claude/process/gates.yaml` as the source of truth for checks.
- Apply `.claude/rules/command-execution.md`.
- Apply `.claude/rules/migration-conventions.md` for project context.
- Apply `.claude/skills/java-springboot/references/migration-implementation-checklist.md` for
  expected progress-artifact fields.
- Apply `.claude/skills/java-springboot/templates/migration-progress-template.md` as the progress
...
- Requested gate checks from `.claude/process/gates.yaml`.
