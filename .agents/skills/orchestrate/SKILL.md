...
This skill drives work through the process declared under `.agents/process/`. It knows how to load
...
| Input | File | Purpose |
|---|---|---|
| Config | `.agents/process/config.yaml` | Selects the active provider adapter. |
| Process spec | `.agents/process/<type>.yaml` | Defines phases, branch patterns, artifacts, and gates. |
| Gates | `.agents/process/gates.yaml` | Defines gate meaning, gate agents, commands, manual stops, and routing. |
| Provider | `.agents/process/provider.<provider>.yaml` | Maps abstract tracker/VCS operations to concrete commands or manual operations. |
| Project conventions | `.agents/rules/project-conventions.md` | Supplies application-specific facts for feature, bug, chore, and docs work. |
| Migration conventions | `.agents/rules/migration-conventions.md` | Supplies migration-specific facts for migration work. |
...
1. Read `.agents/process/config.yaml`, the selected process spec, `.agents/process/gates.yaml`, and
   `.agents/process/provider.<provider>.yaml`.
...
- Add a lifecycle by creating `.agents/process/<type>.yaml`.
- Add a gate by editing `.agents/process/gates.yaml`.
- Add a provider by creating `.agents/process/provider.<provider>.yaml` and selecting it in
  `.agents/process/config.yaml`.
- Add a role under `.agents/roles/` and adapter metadata under `.agents/adapters/`.
